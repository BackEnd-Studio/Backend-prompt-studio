...
import "./styles.css"; // Pastikan file CSS masih di-impor
...
// Ganti blok const firebaseConfig di bawah ini dengan yang kamu salin
...
// --- Database Bilingual (SAMA SEPERTI SEBELUMNYA) ---
const optionData = {
  style: [
    {
      id: "fotorealistis",
      id_text: "Fotorealistis",
      en_text: "Photorealistic",
    },
    { id: "sinematik", id_text: "Sinematik", en_text: "Cinematic" },
    { id: "anime", id_text: "Anime / Manga", en_text: "Anime / Manga" },
    {
      id: "lukisan_cat_minyak",
      id_text: "Lukisan Cat Minyak",
      en_text: "Oil Painting",
    },
  ...
  lighting: [
    ...
    {
      id: "sinematik_lighting",
      id_text: "Pencahayaan Sinematik",
      en_text: "Cinematic lighting",
    },
  ],
  color: [
  ...
  composition: [
    ...
    {
      id: "medium_shot",
      id_text: "Bidikan Setengah Badan",
      en_text: "Medium Shot",
    },
    ...
    {
      id: "low_angle",
      id_text: "Bidikan Sudut Bawah",
      en_text: "Low-Angle Shot",
    },
    {
      id: "high_angle",
      id_text: "Bidikan Sudut Atas",
      en_text: "High-Angle Shot",
    },
  ],
  mood: [
    ...
    {
      id: "sedih",
      id_text: "Sedih / Melankolis",
      en_text: "Sad / Melancholic",
    },
    {
      id: "misterius",
      id_text: "Misterius / Gelap",
      en_text: "Mysterious / Dark",
    },
...
// ... (Komponen OptionButton dan TextInput tetap sama persis) ...
const OptionButton = ({ category, option, selected, onSelect }) => (
  <button
...
function App() {
  ...
  const [activeTab, setActiveTab] = useState("generator"); // 'generator' atau 'favorit'

  // State untuk input
  const [subject, setSubject] = useState("");
  const [background, setBackground] = useState("");
  const [selectedStyle, setStyle] = useState(null);
  const [selectedLighting, setLighting] = useState(null);
  const [selectedColor, setColor] = useState(null);
  const [selectedComp, setComp] = useState(null);
  const [selectedMood, setMood] = useState(null);

  // State BARU untuk daftar favorit
  ...
  const handleSelect = (category, value) => {
    ...
    setters[category]((prev) => (prev === value ? null : value));
  };

  // --- Mesin Perakit Kalimat (SAMA SEPERTI SEBELUMNYA) ---
  const generatedPrompts = useMemo(() => {
    const findText = (category, id) => {
      if (!id) return null;
      const item = optionData[category].find((opt) => opt.id === id);
    ...
    if (
      ...
      return {
        id: "Pilih opsi di atas untuk membuat prompt...",
        en: "Select options above to build a prompt...",
      };
    }
  ...
  const handleSave = async () => {
    // Jangan simpan jika prompt masih default
    if (generatedPrompts.id.startsWith("Pilih opsi")) return;

    setIsLoading(true);
    try {
      // Simpan kedua versi prompt ke koleksi 'prompts'
      await addDoc(collection(db, "prompts"), {
        prompt_id: generatedPrompts.id,
        prompt_en: generatedPrompts.en,
        createdAt: new Date(), // Simpan tanggal dibuat
      });
  ...
  // --- FUNGSI BARU: Ambil data Favorit dari Firebase ---
  const fetchFavorites = async () => {
    setIsLoading(true);
    const querySnapshot = await getDocs(collection(db, "prompts"));
    const favsList = [];
    querySnapshot.forEach((doc) => {
      // Masukkan ID dokumen (untuk hapus) dan datanya
  ...
  // Ambil data favorit saat tab 'favorit' diklik
  useEffect(() => {
  ...
  return (
    <div className={`app-container ${isDarkMode ? "dark-mode" : "light-mode"}`}>
      ...
      <nav className="nav-tabs">
        ...
        <button
          className={`nav-tab ${activeTab === "favorit" ? "active" : ""}`}
      ...
      {activeTab === "generator" && (
        <>
          <main className="generator-body">
            ...
            <div className="input-section">
              <h4>Pilih Gaya Visual</h4>
              <div className="option-buttons">
                {optionData.style.map((opt) => (
                  <OptionButton
                    key={opt.id}
                    category="style"
                    option={opt}
            ...
            <div className="input-section">
              <h4>Pilih Pencahayaan</h4>
              <div className="option-buttons">
                {optionData.lighting.map((opt) => (
                  <OptionButton
                    key={opt.id}
                    category="lighting"
                    option={opt}
                    selected={selectedLighting === opt.id}
                    onSelect={handleSelect}
                  />
                ))}
              </div>
            ...
            <div className="input-section">
              <h4>Pilih Palet Warna</h4>
              <div className="option-buttons">
                {optionData.color.map((opt) => (
                  <OptionButton
                    key={opt.id}
                    category="color"
                    option={opt}
                    selected={selectedColor === opt.id}
                    onSelect={handleSelect}
                  />
                ))}
              </div>
            ...
            <div className="input-section">
              <h4>Pilih Komposisi</h4>
              <div className="option-buttons">
                {optionData.composition.map((opt) => (
                  <OptionButton
                    key={opt.id}
                    category="composition"
                    option={opt}
                    selected={selectedComp === opt.id}
                    onSelect={handleSelect}
                  />
                ))}
              </div>
          ...
          <footer className="output-area">
            ...
            <div className="lang-tabs">
              <button
                className={`lang-tab ${activeLang === "id" ? "active" : ""}`}
      ...
      {/* --- Tampilan Tab Favorit (Opsi 1: Daftar Sederhana) --- */}
      {activeTab === "favorit" && (
        <main className="favorites-body">
          {isLoading && <p>Memuat favorit...</p>}
          {!isLoading && favorites.length === 0 && (
            <p>
              Belum ada prompt yang disimpan. Coba simpan beberapa dari tab
              Generator!
            </p>
          )}
          {!isLoading && favorites.length > 0 && (
              ...
              {favorites.map((fav) => (
                <li key={fav.id} className="favorite-item">
                  ...
                  <div className="fav-actions">
                    <button onClick={() => handleCopy(fav.prompt_id)}>
                      Salin ID
                    </button>
                    <button onClick={() => handleCopy(fav.prompt_en)}>
                      Salin EN
                    </button>
                    {/* Fungsi Hapus (Delete) bisa ditambahkan di sini nanti */}
                  </div>
                </li>
              ))}
            </ul>
